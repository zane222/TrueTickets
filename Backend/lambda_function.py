...
def handle_user_invitation(event, context):
    ...
    try:
        ...
        try:
            ...
            print(
                f"ClientError when checking if user exists: {error_code} - {error_msg}"
            )
            ...
            return {
                ...
                "headers": {"Content-Type": "application/json"},
                "body": json.dumps(
                    {"error": f"Error checking user existence: {error_msg}"}
                ),
            }
        except Exception as e:
        ...
        try:
            ...
            return {
                ...
                "headers": {"Content-Type": "application/json"},
                "body": json.dumps(
                    {"error": f"Could not invite user: {err_msg}", "code": err_code}
                ),
            }
...
def handle_update_user_group(event, context):
    try:
        ...
        current_groups = [
            group["GroupName"] for group in current_groups_response.get("Groups", [])
        ]
        ...
        cognito.admin_add_user_to_group(
            UserPoolId=USER_POOL_ID, Username=username, GroupName=new_group
        )
...
def can_manage_users(user_groups):
    ...
    allowed_groups = ["TrueTickets-Cacell-ApplicationAdmin", "TrueTickets-Cacell-Owner"]
    return any(group in allowed_groups for group in user_groups)
