//! Migration utilities for importing data from a legacy system.
use chrono::Utc;
...
///
/// # Logic
/// - **Concurrency**: Spawns asynchronous tasks using `tokio::spawn` to migrate multiple tickets in parallel (up to `count`).
/// - **Reverse Iteration**: Starts from `latest_ticket_number` and works backwards.
/// - **State Management**: Updates the global `ticket_number_counter` in DynamoDB to ensure the new system resumes numbering *after* the migrated range.
...
/// Migrates a single ticket and its dependencies from the legacy RepairShopr API to DynamoDB.
///
/// # Database & Cloud Interactions
/// 1. **External API**: HTTP GET requests to search and fetch ticket details.
/// 2. **S3**: Downloads attachments from the legacy system and re-uploads them to Amazon S3.
/// 3. **DynamoDB Transactions**:
///    - **Customers**: Creates/Updates customer and phone lookup table entries.
///    - **PhoneNumberIndex**: Adds phone number to index.
///    - **Tickets**: Inserts the fully processed ticket with normalized fields.
///
/// # Logic
/// - **ETL Process**: Extracts data (API), transforms it (field mapping, status conversion, password extraction), and loads it (DynamoDB).
/// - **Attachment Handling**: Migrates files physically to S3 to decouple from the old system.
/// - **Consistency**: Uses a single transaction to ensure the customer and ticket are committed together.
